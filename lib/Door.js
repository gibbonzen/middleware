...
module.exports = class Door extends Device {
	constructor(jsonBuffer) {
		...
		if(this.automate.auto) {
			...
			if(automate.hopen !== undefined) {
				this.tasks.push({
					"open": this.startTask(automate.hopen, this.open, automate.auto)
				})
			}

			if(automate.hclose !== undefined) {
				this.tasks.push({
					"close": this.startTask(automate.hclose, this.close, automate.auto)
				})
	...
	startTask(time, action, start) {
		...
		if(time !== undefined) {
			...
			let job = CronJob(`00 ${minutes} ${hours} * * *`, 
//			return new CronJob('* * * * * *',
					() => action(action.name), 
					() => {},
					false,
	...
	updateTask() {
		...
		if(this.tasks !== undefined && this.tasks.length !== 0) {	
			...
			openTask.stop()
			openTask = this.startTask(this.automate.hopen, this.open, this.automate.auto)

			let closeTask = this.tasks.close
			closeTask.stop()
			closeTask = this.startTask(this.automate.hclose, this.close, this.automate.auto)
		}
	}
